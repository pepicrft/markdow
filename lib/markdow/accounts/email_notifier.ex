defmodule Markdow.Accounts.EmailNotifier do
  ...
  @callback deliver_login_link(User.t(), String.t()) ::
              {:ok, Swoosh.Email.t()} | {:error, term()}
  ...
  @spec deliver_login_link(User.t(), String.t()) :: {:ok, Swoosh.Email.t()} | {:error, term()}
  def deliver_login_link(%User{} = user, url) when is_binary(url) do
    {from_name, from_address} =
      Application.get_env(:markdow, :email_from, {"Markdow", "hello@markdow.org"})
    ...
    email =
      ...
      |> to(user.email)
      |> from({from_name, from_address})
      |> subject("Sign in to Markdow")
      |> text_body(login_text_body(url))
      |> html_body(login_html_body(url))
  ...
  defp login_text_body(url) do
    """
    Sign in to Markdow

    Open this secure link to sign in:

    #{url}

    The link expires in 15 minutes and can only be used once. If you did not request it, ignore this message.
    """
  end
      ...
      <body style="margin:0;padding:24px;background:#f8f7f2;color:#24231f;font-family:Georgia,'Times New Roman',serif;font-size:16px;line-height:1.55;">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:520px;margin:0 auto;">
          <tr>
            <td>
              <p style="margin:0 0 24px;font-family:Helvetica,Arial,sans-serif;font-size:13px;letter-spacing:.08em;text-transform:uppercase;color:#6e6b62;">Markdow</p>
              <h1 style="margin:0 0 16px;font-family:Helvetica,Arial,sans-serif;font-size:28px;line-height:1.2;">Sign in to Markdow</h1>
              <p style="margin:0 0 24px;">Use this one-time link to sign in and continue to your vaults.</p>
              <p style="margin:0 0 24px;"><a href="#{href}" style="display:inline-block;padding:14px 24px;background:#285d4b;color:#f5f3eb;font-family:Helvetica,Arial,sans-serif;font-size:15px;font-weight:600;text-decoration:none;">Sign in to Markdow</a></p>
              <p style="margin:0 0 8px;font-family:Helvetica,Arial,sans-serif;font-size:13px;color:#6e6b62;">If the button does not work, open this address in your browser:</p>
              <p style="margin:0 0 24px;font-family:Helvetica,Arial,sans-serif;font-size:13px;word-break:break-all;"><a href="#{href}" style="color:#285d4b;">#{href}</a></p>
              <p style="margin:0;padding-top:16px;border-top:1px solid #cfccc2;font-family:Helvetica,Arial,sans-serif;font-size:13px;color:#6e6b62;">The link expires in 15 minutes and can only be used once. If you did not request it, ignore this message.</p>
            </td>
          </tr>
        </table>
      </body>
